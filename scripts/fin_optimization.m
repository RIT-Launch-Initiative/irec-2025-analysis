...
%  The "best" design is now the one that meets constraints and is closest to 10k feet.
...
for i = 1:num_elements
    ...
    otis.save();  % Optionally save the .ork after each iteration (can be expensive)
    sim = otis.sims(sim_name);
    opts = sim.getOptions;
    opts.setWindSpeedDeviation(wind_speed_deviation);
    opts.setTimeStep(time_step);
    ...
    % Apogee [m] → we will convert to ft for reporting
...
%% 6. TRIM RESULTS & FIND THE "BEST" DESIGN (APOGEE CLOSEST TO 10,000 ft)
...
% Find the design with apogee closest to 10,000 ft (3048 m)
...
disp(" ");
disp("BEST DESIGN (Apogee closest to 10,000 ft):");
...
% Set rocket to best-design geometry (closest to 10k ft) & save
