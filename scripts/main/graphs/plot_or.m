...
base_sim = otis.sims("0MPH-TEXAS-30C"); 
openrocket.simulate(base_sim);
data = openrocket.get_data(base_sim);
...
plot_openrocket(data, "Altitude", "Total velocity", ...
    end_ev = "APOGEE", labels = ["BURNOUT", "APOGEE", "MAIN"]);

figure
data.("Angle of attack (deg)") = rad2deg(data.("Angle of attack"));
plot_openrocket(data, "Stability margin", "Angle of attack (deg)", ...
    start_ev = "LAUNCHROD", end_ev = "BURNOUT", labels = ["LAUNCHROD", "BURNOUT"]);
