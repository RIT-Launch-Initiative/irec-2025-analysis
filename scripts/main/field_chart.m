%% mass_sizing_chart.m
%% Author: ChatGPT draft — refine as needed
% Calculates the required adjustable‑nose mass to hit a target apogee
% (10 150 ft AGL) over a grid of ambient temperature and wind speed.
% -------------------------------------------------------------------------
% CRITICAL ASSUMPTIONS
%   • Apogee is monotonic ↓ with increasing nose‑cone mass.
%   • Atmospheric density depends only on temperature (pressure fixed).
%   • Wind is steady (no gust model), on‑rail losses neglected.
% If any of those fail in reality, the bisection search below may not
% converge or give unique answers—run spot checks!
% -------------------------------------------------------------------------
...
for kT = 1:nT
    ...
    for jW = 1:nW
        ...
        for iter = 1:maxIter
            mid = 0.5*(lo+hi);
            f_mid = f(mid);
            if abs(f_mid) < tol_ft || (hi-lo) < 0.01
                break;     % converged
            end
...
%% ── PLOT SIZING CHART ────────────────────────────────────────────────────
[T_mesh, W_mesh] = meshgrid(tempVals_C, windVals_mph);  % note: meshgrid transposed vs. loops
T_mesh = T_mesh'; W_mesh = W_mesh';

fig = figure('Color','w');
levels = noseMin_kg : 0.5 : noseMax_kg;
...
title(sprintf('Mass required for %.0f ft AGL | Midland launch', targetApogee_ft));

%% ── OPTIONAL SAVES ───────────────────────────────────────────────────────
% exportgraphics(fig, 'mass_sizing_chart.png', 'Resolution', 300);
% save('massChart.mat', 'massChart_kg', 'tempVals_C', 'windVals_mph');
...
function diff = apogeeDiff(mass_kg, T_K, W_ms, ft2m, target_ft, otis, NoseWeight, simName)
    ...
    NoseWeight.setMassOverridden(mass_kg);
